...
class SettingsController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    fileprivate func setupNavigationItems() {
        ...
        let image_save = UIImage(named: "ic_save")?.withRenderingMode(.alwaysOriginal)
        let button_save = UIBarButtonItem(image: image_save, style: .plain, target: self, action: #selector(handleSave))
        button_save.tintColor = .mainBlack()
        
        let image_logout = UIImage(named: "ic_security")?.withRenderingMode(.alwaysOriginal)
        let button_logout = UIBarButtonItem(image: image_logout, style: .plain, target: self, action: #selector(handleLogOut))
        button_logout.tintColor = .mainBlack()
...
//        ]
    ...
    @objc fileprivate func handleSave() {
        ...
        hud.show(in: view)
        Firestore.firestore().collection("users").document(uid).collection("profile").document(uid).setData(docData) { (err) in
            hud.dismiss()
            if let err = err {
