...
//  maple_release
//
//  Created by Murray Toews on 2018/04/30.
...
final class NotificationCollection<T: NotificationSerializable> {
...
final class LocalCollection<T: DocumentSerializable> {
    ...
    func listen() {
        ...
        listener = query.addSnapshotListener { [unowned self] querySnapshot, error in
            ...
            let models = snapshot.documents.map { (document) -> T in
                if let model = T(dictionary: document.data(), postId: document.documentID ) {
                    return model
                } else {
                    // handle error
                    fatalError("Unable to initialize type \(T.self) with dictionary \(document.data())")
